...
# 2. 运行安装脚本（会弹出菜单：1.安装 2.卸载 3.更新 4.检查 5.版本）
bash scripts/install.sh
# 选择 1 安装，脚本会自动创建虚拟环境、安装依赖、交互式配置 API Key
...
- **顶部标题栏**：右上角有服务状态灯。**绿点**表示后端 API 连通正常，**红点**表示连接异常（状态每 20 分钟自动真实探测一次）。
- **左侧上传区**：三种上传方式——拖拽图片、点击选择、复制图片后点「粘贴剪贴板图片」按钮（`Ctrl+V`）。下方有「开始检测 / 重新上传 / 清空」按钮。
- **右侧日志面板**：检测时实时显示进度（校验 → 模型分析 → 结果解析 → 综合判定）。
- **底部报告区**：显示三个指标、综合评价、检测依据；最下方是最近检测记录。

历史记录保存在**浏览器本地**（localStorage），刷新页面不会丢失，点「清空检测记录」可手动清除。
...
**解决**: 状态灯通过向 API 发送轻量测试请求探测连通性。若检测能正常出结果但状态灯为红，通常是探测请求被模型接口拒绝（部分视觉模型对纯文本请求有限制）。可先忽略；如需精确状态，请反馈以便调整探测方式。演示模式（`APP_MODE=demo`）下状态灯恒为绿色。

### 问题6: 粘贴上传按钮没有出现 / 历史记录刷新丢失

**解决**: 这两个功能依赖可选组件。确保已安装完整依赖：
```bash
pip install -r requirements.txt
```
若组件缺失，应用会自动降级——粘贴按钮隐藏、历史退回会话内存储，核心检测不受影响。
